...
const charToValue = { "T": 10, "J": 1, "Q": 11, "K": 12, "A": 13 };

for (let i = 2; i <= 9; i++ ) {
    charToValue[i.toString()] = i;
}
...
function jokerify(str, r) {
    return str.replaceAll('J', r);
}
...
function getTypeValueOfHand(hand) {
    if (mem[hand]) return mem[hand];
    let value = 0;

    typeClassifier.forEach((v, i) => {
        for (const card in charToValue) {
            if (v(jokerify(hand, card))) {
                value = i+1;
            }   
        }
    })
...
function sortHand(hands) {
    ...
    while (hands.length > 0) {
        ...
        ans.unshift(highest);
        console.log(highest, hands.length);
